    ...
    <Footer>
      <div className="svcs">
        <div className="svc">
          ...
          <h2>Что это</h2>
            <p><a href="http://jc.jabber.ru">jc.jabber.ru</a> -  первая в России система рейтинга и поиска джаббер-конференций, которая собирает и обрабатывает статистику с ведущих джаббер-серверов Рунета, а так же предоставляет интерфейс для удобного поиска по ним.</p>
            <p>Если вы тут первый раз и не поняли о чем шла речь в первом абзаце, то данный азбац как раз для вас. На самом деле все просто! Джаббер-конференция - это тот же чат(chat), т.е. место коллективного общения, если совсем в двух словах. А сам джаббер - это название протокола обмена мгновенными сообщениями, также известный как XMPP, который был принят в сети как стандарт, по этому сегодня его используют все ведущие ресурсы Рунета для обмена сообщениями.</p>
            <p>Данный проект создавался для того, чтоб облегчить пользвателям выбор места для общения, так называемых чат-комнат(конференций). На нашем сайте Вы можете воспользоваться удобным поиском, посмотреть рейтинг самых посещаемых конференций, найти информацию о программном обеспечении и других сопутсовующих вещах.</p>
          ...
          <h2>У вас есть сервер конференций и вы хотите участвовать в рейтиге</h2>
            <p>Если конференции вашего сервера посещаемы, то воспользуйтесь формой обратной связи и оставьте нам сообщение. Для участия в рейтинге ваш сервер должен позволять получать информацию с акаунтов jabber.ru через Discovery.</p>
        </div>
        </div>
        ...
        <div className="svc">
          ...
          <p>Сейчас juick работает с жаббером и телеграмом. С watsapp'ом и viber'ом не работает по очевидным причинам:закрытый протокол и несговорчивость в создании гейтов в другие сервисы.</p>
        ...
        <div className="svc planning">
          ...
          <h2>Что именно умеет jabber.ru</h2>
            <p>Jabber. Мы умеет jabber, мгновенные сообщения. В будущем мы планируем запустить виртуальный почтовый сервис. А так же станцию слежения за астероидами в поясе Койпера. Но только после захвата мира.</p>
          <h2>Как начать использовать</h2>
            <p>Чтобы использовать сервера jabber.ru со своим доменом вам потребуется зарегистрировать учётную запись на нашем сервисе и настроить ДНС. После подверждения учётной записи, вы сможете зарегистрировать в системе перенаправление ваших доменов на сервера jabber.ru. После проверки нашим сервисом ДНС записей - всё готово, вы можете добавлять свои пользователей и начинать общение под собственным доменным именем.
</p>
          <h2>Дополнительные услуги</h2>
            <p>Если у вас есть идеи связанные с jabber, а наш сервис их, на первый взгляд, не поддерживает, — напишите нам в support@conference.jabber.ru, и мы обязательно что-нибудь придумаем в индивидуальном порядке. Мы любит странные штуки и беспокойных людей.</p>       
        </div>
        </div>
      </div> 
    </Footer>
